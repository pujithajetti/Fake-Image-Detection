...
def get_model_info():
    ...
    return {
        "VGG16": {"accuracy": 95.27, "description": "Deep CNN with 16 layers"},
        "VGG19": {"accuracy": 95.21, "description": "Deep CNN with 19 layers"},
        "ResNet50": {"accuracy": 94.00, "description": "Residual network with 50 layers"},
        "InceptionV3": {"accuracy": 77.20, "description": "Inception architecture for efficient recognition"},
...
def main():
    ...
    st.success(f"✅ Loaded 6 models successfully!")
    ...
    if selected_model == "Ensemble":
        st.sidebar.subheader("📊 Ensemble Information")
        st.sidebar.metric("Expected Accuracy", "96.80%")
        st.sidebar.write("**Description:** Weighted combination of all models")
        st.sidebar.write("**Models:** VGG16, VGG19, ResNet50, InceptionV3, Custom")
        st.sidebar.write("**Weights:** VGG16(25%), VGG19(25%), ResNet50(20%), InceptionV3(15%), Custom(15%)")
    elif selected_model in model_info:
        ...
        if uploaded_file is not None:
            ...
            if st.button(f"🔍 Analyze with {selected_model}", type="primary"):
                with st.spinner(f"Analyzing image with {selected_model}..."):
                    if selected_model == "Ensemble":
                        # Run all models and create ensemble prediction
                        all_results = []
                        for model_name, model in models.items():
                            result = predict_with_model(model, model_name, image_array)
                            if result:
                                all_results.append(result)
                        
                        if all_results:
                            ensemble_result = ensemble_prediction(all_results)
                            if ensemble_result:
                                # Store ensemble result in session state
                                st.session_state['selected_model'] = selected_model
                                st.session_state['result'] = ensemble_result
    ...
    with col2:
        ...
        if 'result' in st.session_state and 'selected_model' in st.session_state:
            ...
            if model_name == "Ensemble":
                ...
                with col_acc:
                    st.metric("Expected Accuracy", "96.80%")
                with col_desc:
                    st.write("**Weighted combination of all models**")
            ...
            # Show individual model results for ensemble
            if model_name == "Ensemble" and 'all_results' in st.session_state:
                st.subheader("📊 Individual Model Results")
                all_results = st.session_state['all_results']
                
                # Create a table of individual results
                import pandas as pd
                results_data = []
                for res in all_results:
                    results_data.append({
                        ...
                        'Fake Prob': f"{res['fake_probability']:.3f}",
                        'Real Prob': f"{res['real_probability']:.3f}"
                    })
                ...
                fake_votes = sum(1 for r in all_results if r['prediction'] == 'FAKE')
                real_votes = len(all_results) - fake_votes
                
                st.write(f"**Model Agreement:** {fake_votes} models say FAKE, {real_votes} models say REAL")
                
                if fake_votes > real_votes:
                    st.info("🎯 Majority vote: FAKE")
                elif real_votes > fake_votes:
                    st.info("🎯 Majority vote: REAL")
                else:
                    st.warning("🎯 Majority vote: TIE")
